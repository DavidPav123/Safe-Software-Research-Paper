...
for i in range(3):
    subprocess.call(f"valgrind --tool=massif --massif-out-file=\"/workspaces/Rusty-Kernels-Code/read-write/C/Results/massif{i}.txt\" --stacks=yes /workspaces/Rusty-Kernels-Code/read-write/C/read_fileC", shell=True, executable="/bin/bash", stdout=subprocess.PIPE)
    os.remove("/workspaces/Rusty-Kernels-Code/read file/destination.txt")
