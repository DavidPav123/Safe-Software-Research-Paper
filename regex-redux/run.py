...
for i in range(5):
    subprocess.call(f"valgrind --tool=massif --massif-out-file=\"/workspaces/Rusty-Kernels-Code/regex-redux/C/Results/massif{i}.txt\" --pages-as-heap=yes /workspaces/Rusty-Kernels-Code/regex-redux/C/regexreduxC < /workspaces/Rusty-Kernels-Code/regex-redux/input.txt > /dev/null", shell=True, executable="/bin/bash", stdout=subprocess.PIPE)
...
for i in range(5):
    subprocess.call(f"valgrind --tool=massif --massif-out-file=\"/workspaces/Rusty-Kernels-Code/regex-redux/Rust/Results/massif{i}.txt\" --pages-as-heap=yes /workspaces/Rusty-Kernels-Code/regex-redux/Rust/target/release/Rust < /workspaces/Rusty-Kernels-Code/regex-redux/input.txt > /dev/null", shell=True, executable="/bin/bash", stdout=subprocess.PIPE)
